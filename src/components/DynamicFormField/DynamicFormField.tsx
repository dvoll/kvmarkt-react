...
const DynamicFormField: React.SFC<DynFormTypes> = (props) => {
    ...
    switch(props.type) {
        case DynamicFormInputTypes.SELECT:
            const items = props.items && props.items.map(item => <option key={item.value} value={item.value}>{item.name}</option>)
            element= (<select {...props as React.DetailedHTMLProps<React.SelectHTMLAttributes<HTMLSelectElement>, HTMLSelectElement>} >
                { items }
            </select>);
            break;
        ...
        case DynamicFormInputTypes.SUBMIT:
            element = <BaseInput {...props as React.DetailedHTMLProps<React.InputHTMLAttributes<HTMLInputElement>, HTMLInputElement>} type={props.type} />;
            break;
        case DynamicFormInputTypes.TEXTAREA: 
            element = <textarea {...props as React.DetailedHTMLProps<React.InputHTMLAttributes<HTMLTextAreaElement>, HTMLTextAreaElement>} />;
            break;
        default: 
